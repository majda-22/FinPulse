...
def test_openapi_registers_expected_paths():
    ...
    assert "/api/v1/companies/ticker-by-name" in schema["paths"]
    assert "/api/v1/companies/tickers" in schema["paths"]
    assert "/api/v1/pipelines/backfill/company" in schema["paths"]
    assert "/api/v1/pipelines/signals/company" in schema["paths"]
    assert "/api/v1/pipelines/signals/filing/{filing_id}" in schema["paths"]
    assert "/api/v1/pipelines/jobs/{job_id}" in schema["paths"]
    assert "/api/v1/score/{ticker}" in schema["paths"]
    assert "/api/v1/score/{ticker}/value/{field_name}" in schema["paths"]
    assert "/api/v1/score/{ticker}_get_{field_name}" in schema["paths"]
    ...
    assert "/api/v1/embeddings/{ticker}/latest/value/{field_name}" in schema["paths"]
    assert "/api/v1/embeddings/{ticker}/latest_get_{field_name}" in schema["paths"]
